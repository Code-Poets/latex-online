...
var utils = {
    ...
    /**
    * Execute shell command with specified params.
    * Note: Avoid params with "=" string (ex `--command-option=...`). They are not served well.
    * @param {string} shellScript Script's path
    * @param {string[]} scriptParams Parameters of script
    ...
    */
    executeCommand: function(cmd, params) {
        ...
        var child = exec(
          cmd,
          params,
          createErrorHandler(`ERROR: failed to execute ${cmd + ' ' + params.join(' ')}.`, fulfill)
        );
        return promise;
    },
