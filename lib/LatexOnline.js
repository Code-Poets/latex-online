...
class LatexOnline extends EventEmitter {
    ...
    * @return {!Promise<!LatexOnline.Result>}
    */
    async prepareJsonCompilation(jsonData, command) {
        var {request, userError} = await CompilationRequest.createForJson(jsonData, command);
        if (!request)
            return new LatexOnline.Result(null, null, userError);
        var downloader = this._downloadManager.createJsonDownloader(jsonData, request.target);
        return new LatexOnline.Result(request, downloader, null);
  }
}
